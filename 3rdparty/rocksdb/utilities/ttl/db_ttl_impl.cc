...
Status DBWithTTLImpl::AppendVersionAndExpiredTime(const Slice& val, std::string* val_with_ver_ts, Env* env, int32_t version, int32_t expired_time) {
...
Status DBWithTTLImpl::AppendVersionAndKeyTTL(const Slice& val, std::string* val_with_ver_ts, Env* env, int32_t version, int32_t ttl) {
  val_with_ver_ts->reserve(kVersionLength + kTSLength + val.size());
...
Status DBWithTTLImpl::SanityCheckTimestamp(const Slice& str) {
  if (str.size() < kTSLength + kVersionLength) {
    ...
    public:
      ...
      virtual Status PutCF(uint32_t column_family_id, const Slice& key, const Slice& value) {
        ...
        if (!st.ok()) {
          ...
          WriteBatchInternal::Put(&updates_ttl, column_family_id, key, value_with_ver_ts);
        }
        return Status::OK();
      }
    ...
    public:
      ...
      virtual Status MergeCF(uint32_t column_family_id, const Slice& key,
        ...
        if (!st.ok()) {
          ...
          WriteBatchInternal::Merge(&updates_ttl, column_family_id, key, value_with_ver_ts);
        }
        return Status::OK();
      }
      virtual Status DeleteCF(uint32_t column_family_id, const Slice& key) {
...
Iterator* DBWithTTLImpl::NewIterator(const ReadOptions& opts, ColumnFamilyHandle* column_family) {
